...
function New-TdpscPersonImport
{
    ...
    Process
    {
        if ($PSCmdlet.ShouldProcess($Bearer, 'Gets a list of users.'))
        {
            ...
            $request = Get-UploadBoundaryEncodedByte @param
            $result = Invoke-WebRequest -Uri $url -Body $request -ContentType "multipart/form-data; boundary=`"$($param.Boundary)`"" -Method Post -Headers @{'Authorization' = 'Bearer ' + $Bearer;} -UseBasicParsing
